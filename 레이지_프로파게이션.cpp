...
long long sum(int cur,int obj_left, int obj_right, int left, int right) {

	if (lazy[cur] != 0) prop(cur, left, right);
	if (obj_right < left || right < obj_left) return 0;
	if (obj_left <= left && right <= obj_right) return tree[cur];

	int mid = (left + right) / 2;
	return sum(cur * 2 + 1, obj_left, obj_right, left, mid) 
			+ sum(cur * 2 + 2, obj_left, obj_right, mid+1, right);
}
int main() {
	int n,q1,q2;
	scanf("%d %d %d", &n, &q1, &q2);
